...
import net.frogmouth.rnd.jim.npif.tables.sensordata.SensorDataTable;
import net.frogmouth.rnd.jim.npif.tables.sensorparametric.PassiveSensorDescriptionDataTable;
...
public class ReaderTest extends TestSupport {
    ...
    @Test
    public void goldenFrame24() throws IOException {
        if (haveGoldenFiles()) {
            Reader reader = new Reader(getGoldenFile("frame-24"));
            assertNotNull(reader);
            assertEquals(reader.getTableTables().size(), 728);
            WritableImage image = render(reader.getTableTables());
            ImageIO.write(SwingFXUtils.fromFXImage(image, null), "png", new File("frame-24.png"));
        }
    }
    ...
    private WritableImage render(List<DataTable> tables) throws IOException {
        ...
        for (DataTable table : tables) {
            if (table instanceof SensorDataTable) {
                ...
                if (sensorData.getHeader().isCompressionFlag()) {
                    // TODO
                    BufferedImage bufferedImage = ImageIO.read(new ByteArrayInputStream(data));
                    image = SwingFXUtils.toFXImage(bufferedImage, image);
                } else {
                    assert (data.length
                            == numColumns * numRowsPerFrame * numBitsPerPixel / Byte.SIZE);
